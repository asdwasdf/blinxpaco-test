...
3. Resume first valid unfinished phase or validate requested phase. `LOCATE` is required unless a non-UI scope records an exact skip reason. Khi có video, INGEST phải tạo/review contact sheet và timeline trước ANALYZE.
4. Search generated product graph trước LOCATE. Dùng Claude Playwright plugin-first. Trên configured dev host, cho phép full mutation trong approved scope qua hostname-aware gate và mutation ledger; production luôn bị chặn.
...
6. For `LOCATE`, require environment, role và valid manual auth; treat tester location clue as optional. Check `docs/product/survey/graph.json` và `feature-map.md` first, validate reusable route in 1–3 meaningful views, otherwise bounded scan from dashboard. Bound chống runaway và tạo checkpoint, không phải deadline giảm correctness.
7. **Ask QA early** ngay khi thiếu role/test data/domain rule/expected result, video mâu thuẫn hoặc unreadable, auth lỗi, hoặc behavior thiếu baseline. Gửi blocker, observation, evidence/timestamp, decision cần QA, concrete choices và affected cases.
8. Validate `ChildSkillOutcome`, phase-mode ownership, path, existence, checksum, mutation ledger và durable evidence promotion.
